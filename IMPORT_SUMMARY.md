...
#### 2. **Violation CSV Import** (`POST /api/imports/violations/csv`)
...
curl -X POST http://localhost:8000/api/imports/violations/csv \
  -F "csv_content=violation_number,title,description
CPSC-2024-001,Recall: Graco Stroller,Choking hazard" \
  -F "delimiter=," \
  -F "has_header=true" \
  -F "agency_name=CPSC" \
