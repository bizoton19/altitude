...
async def list_product_bans(
    ...
    """
    try:
        product_bans = await db.get_all_product_bans()
...
async def get_risk_summary():
    ...
    return await db.get_product_bans_risk_summary()
...
async def search_product_bans(
    ...
    """
    product_bans = await db.search_product_bans(q, risk_level, agency_name, country)
...
async def get_product_ban(product_ban_id: str):
    ...
    product_ban = await db.get_product_ban(product_ban_id)
    if not product_ban:
...
async def get_product_ban_listings(product_ban_id: str):
    ...
    listings = await db.get_listings_for_product_ban(product_ban_id)
    return listings
...
async def create_product_ban(
    ...
    """
    from app.services.workflow_service import process_product_ban_import
    from app.models.import_models import ImportSource
    
    result = await process_product_ban_import(
        product_ban_data=product_ban_data,
...
async def delete_all_product_bans():
    ...
    count = await db.delete_all_product_bans()
    return {
...
async def get_product_bans_by_agency(agency_name: str):
    ...
    product_bans = await db.get_product_bans_by_agency(agency_name)
    return product_bans
