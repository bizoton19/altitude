# Product Ban-Product Relationship Analysis

## Current Structure

**Current Model**: One-to-Many (Product Ban → Products)
- `ProductBan` has `products: List[ProductBanProduct] = []`
- Database: Separate `product bans` and `product_ban_products` tables with foreign key
...
- **Relationship Types**:
  - **One-to-One**: Single product, single product ban (most common)
  - **One-to-Many**: Single product ban affects multiple products (manufacturing process issues)
  - **Many-to-One**: Multiple product bans for one product (e.g., contamination + labeling issues)
...
- **Relationship Types**:
  - **One-to-One**: Single product, single product ban (most common)
  - **One-to-Many**: Single product ban affects multiple products (product line issues, shared defects)
  - **Many-to-One**: Multiple product bans for one product over time (different issues)
...
def get_primary_product(product_ban: ProductBan) -> Optional[ProductBanProduct]:
    return product_ban.products[0] if product_ban.products else None

# Check if single product
def is_single_product(product_ban: ProductBan) -> bool:
    return len(product_ban.products) == 1
```
...
- ❌ Would need to duplicate product ban data for each product
...
- Store primary product inline in product ban table
...
2. **Database indexing**: Ensure `product_ban_products.product_ban_id` is indexed
